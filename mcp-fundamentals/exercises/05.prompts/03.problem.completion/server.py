"""EpicMe MCP server — 05.prompts / completion (problem)."""
...
# This prompt does the fetching itself, so the model receives the entry and the
# tag list already in hand. Same argument as before, much less round-tripping.
@mcp.prompt(
    name="suggest_tags",
...
def suggest_tags(
    ...
    return [
        base.UserMessage(
            "Below are a journal entry and every tag that currently exists in "
            "the journal. Suggest which existing tags fit this entry, and any "
            "new tags worth creating. Show me your suggestions and wait for my "
            "approval. Once I approve, create any new tags with the "
            "`create_tag` tool, then attach every approved tag to the entry "
            "with the `add_tag_to_entry` tool."
        ),
...
# TODO: Complete the prompt's `entry_id` argument.
#
# The handler below already completes the `{id}` slot in the resource
# templates, so `epicme://entries/<TAB>` offers real ids. But `suggest_tags`
# takes an `entry_id`, and right now a user picking that prompt has to know
# the id by heart.
#
# This is the one completion handler for the whole server, so the first thing
# it does is work out *what* is being completed. `ref` is either a
# PromptReference (a prompt argument) or a ResourceTemplateReference (a slot
# in a template URI).
#
# Add a branch at the top:
#
#     if isinstance(ref, PromptReference):
#         if ref.name == "suggest_tags" and argument.name == "entry_id":
...
#         return None
#
# Returning None means "no suggestions for this one", which is always a fine
# answer for a ref you don't recognise.
@mcp.completion()
async def handle_completion(
    ...
    if isinstance(ref, ResourceTemplateReference) and argument.name == "id":
