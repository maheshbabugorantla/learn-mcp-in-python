"""EpicMe MCP server — 05.prompts / prompts (problem).

Your job: add a prompt a person can pick from a menu.
Run `uv run pytest exercises/05.prompts/01.problem.prompts` until it passes.
"""
...
# TODO: Register a prompt with `@mcp.prompt()`.
#
# Name it `suggest_tags` and give it a `description=` — the description is what
# a human reads in the menu before picking it, so write it for a person:
#
#     @mcp.prompt(
#         name="suggest_tags",
#         description="Suggest tags for a journal entry",
#     )
#     def suggest_tags(entry_id: ...) -> str:
#         return f"..."
#
# A few things to get right:
#
#   1. The function takes one parameter, `entry_id`. FastMCP turns your
#      parameters into the argument list the client uses to build its input
#      form, and a parameter with no default is a required argument.
#   2. Prompt arguments are **strings on the wire** — the client sends
#      `{"entry_id": "1"}`, never `{"entry_id": 1}`. So annotate it as a `str`
#      and convert with `int(...)` if you ever need a number.
#   3. Describe the argument so the client can label the input:
#      `Annotated[str, Field(description="The ID of the journal entry ...")]`
#   4. Return a plain string. FastMCP wraps it into a single user message.
