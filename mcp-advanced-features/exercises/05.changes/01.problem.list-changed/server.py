...
something. When that happens the client is holding a stale list, so the spec
gives the server a way to say "my tools changed, ask me again":
`notifications/tools/list_changed`.

The lesson is `enable_beta_tools`: it registers a brand-new tool at runtime with
`mcp.add_tool(...)`, then fires `ctx.session.send_tool_list_changed()` so the
client knows to re-fetch. Everything else is the base journal.
"""
...
async def enable_beta_tools(ctx: Context) -> CallToolResult:
    ...
    # 🐨 the tool is registered, but the client is still holding the old list.
    #    Tell it the list changed so it knows to re-fetch:
    #      await ctx.session.send_tool_list_changed()
    raise NotImplementedError("Announce the tool-list change with ctx.session.send_tool_list_changed()")
