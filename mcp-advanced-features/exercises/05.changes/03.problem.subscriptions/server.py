...
async def _subscribe(uri: AnyUrl) -> None:
    # 🐨 remember that this client wants updates for `uri`:
...
async def _unsubscribe(uri: AnyUrl) -> None:
    # 🐨 forget it — the client no longer cares about `uri`:
...
async def _notify_entry_changed(ctx: Context, entry_id: int) -> None:
    ...
    # 🐨 only notify if someone subscribed to THIS uri, then send the update:
    #      if uri in _subscribed:
    #          await ctx.session.send_resource_updated(uri=AnyUrl(uri))
    raise NotImplementedError("Notify subscribers with ctx.session.send_resource_updated(...)")
