"""EpicMe MCP server — sampling (simple, solution).
...
to generate some text and hands the answer back. Here, right after we create a
journal entry, we ask the model for a one-line encouraging reflection and tack it
onto the result.

Sampling is optional on the client. A caller with no sampling support can't
answer, so we wrap the request in try/except: no reflection, but the entry still
...
async def _reflect(ctx: Context, entry: Entry) -> str | None:
    ...
    Returns the model's text, or None if the client can't sample. A client with
    no sampling capability answers `create_message` with an error, so we treat
    any failure as "no reflection" rather than letting it fail the tool.
    """
    try:
