"""EpicMe MCP server — sampling (advanced, solution).

The simple step used sampling for a throwaway reflection. Here we make it earn
its keep: after creating an entry, we ask the client's LLM to suggest 1-3 tag
names for it, then actually create and attach those tags. The model's text drives
a real database change — that's the payoff of sampling.

Two wrinkles the code has to handle: the model returns free text, so we parse the
comma-separated list ourselves; and a suggested tag may already exist (the schema
makes tag names unique), so we find-or-create by name rather than blindly insert.
`create_entry` is the lesson; everything else is the base journal.
"""
