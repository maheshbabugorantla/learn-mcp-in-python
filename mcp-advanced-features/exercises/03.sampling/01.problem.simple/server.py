"""EpicMe MCP server — sampling (simple, problem).

A normal tool call runs one direction: the client asks the server to do work.
Sampling runs the *other* direction — mid-tool, the server asks the client's LLM
to generate some text and hands the answer back. Your job: right after we create
a journal entry, ask the model for a one-line encouraging reflection and tack it
onto the result.

Sampling is optional on the client. A caller with no sampling support can't
answer, so the request lives in a try/except: no reflection, but the entry still
gets created. `create_entry` is the lesson; everything else is the base journal.
"""
...
async def _reflect(ctx: Context, entry: Entry) -> str | None:
    """Ask the client's LLM for a one-line reflection on the new entry.

    Should return the model's text, or None if the client can't sample.
    """
    # 🐨 ask the client's model to generate a reflection on this entry:
    ...
    #    )
    # 🐨 return the model's text: return getattr(res.content, "text", None)
    # 💣 delete this once you do (the reflection test drives it):
    raise NotImplementedError("Ask the model for a reflection with ctx.session.create_message (see the TODO).")
...
async def create_entry(
    ...
    """Create a new journal entry, with an LLM-generated reflection when available."""
    ...
    # A client with no sampling capability answers create_message with an error,
    # so we treat any failure as "no reflection" rather than failing the tool.
    try:
