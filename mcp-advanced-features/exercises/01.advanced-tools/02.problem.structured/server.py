"""EpicMe MCP server — structured output (problem).

Builds on the annotated journal. So far every tool has answered with a
`CallToolResult` full of text and resource links — fine for a human reading
along, but a client that wants the *data* has to parse a JSON string back out
of a text block.

Structured output fixes that. Give a tool a pydantic return type and FastMCP
fills in BOTH halves of the response: `result.structuredContent` (the data as a
real dict, validated against the tool's output schema) AND a text-fallback
block for clients that only read text. Your job: upgrade `get_entry` and
`get_tag` to return the pydantic models below, keeping their read-only
annotations.
"""
...
class EntryData(BaseModel):
    """The structured shape a client should get back from get_entry."""
...
class TagData(BaseModel):
    """The structured shape a client should get back from get_tag."""
...
@mcp.tool(
    annotations=ToolAnnotations(title="Create Entry", destructiveHint=False)
)
def create_entry(
...
@mcp.tool(
    annotations=ToolAnnotations(title="Get Entry", readOnlyHint=True, openWorldHint=False)
)
def get_entry(id: Annotated[int, Field(description="The entry ID")]) -> CallToolResult:
    ...
    # 🐨 Return structured data instead of a text-only CallToolResult:
    #    change this function's return annotation to `-> EntryData` and return
    #    `EntryData(id=entry.id, title=entry.title, content=entry.content, mood=entry.mood)`.
    #    FastMCP will then populate result.structuredContent AND a text fallback.
    # 💣 As long as this returns a CallToolResult, structuredContent stays None
    #    and the test fails.
    return CallToolResult(content=[_text(entry.to_dict()), _entry_link(entry)])


@mcp.tool(
    annotations=ToolAnnotations(title="List Entries", readOnlyHint=True, openWorldHint=False)
)
...
@mcp.tool(
    annotations=ToolAnnotations(title="Update Entry", idempotentHint=True, destructiveHint=False)
)
def update_entry(
...
@mcp.tool(
    annotations=ToolAnnotations(title="Delete Entry", destructiveHint=True, idempotentHint=True)
)
...
@mcp.tool(
    annotations=ToolAnnotations(title="Create Tag", destructiveHint=False)
)
def create_tag(
...
@mcp.tool(
    annotations=ToolAnnotations(title="Get Tag", readOnlyHint=True, openWorldHint=False)
)
def get_tag(id: Annotated[int, Field(description="The tag ID")]) -> CallToolResult:
    ...
    # 🐨 Return structured data instead of a text-only CallToolResult:
    #    change this function's return annotation to `-> TagData` and return
    #    `TagData(id=tag.id, name=tag.name, description=tag.description)`.
    # 💣 As long as this returns a CallToolResult, structuredContent stays None
    #    and the test fails.
...
@mcp.tool(
    annotations=ToolAnnotations(title="Update Tag", idempotentHint=True, destructiveHint=False)
)
def update_tag(
...
@mcp.tool(
    annotations=ToolAnnotations(title="Add Tag to Entry", destructiveHint=False)
)
