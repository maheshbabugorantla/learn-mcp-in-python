"""EpicMe MCP server — structured output (solution).
...
block for clients that only read text. `get_entry` and `get_tag` are the two
tools we upgrade here; they keep their read-only annotations.
"""
...
class EntryData(BaseModel):
    """The structured shape a client gets back from get_entry."""
...
class TagData(BaseModel):
    """The structured shape a client gets back from get_tag."""
...
def get_entry(id: Annotated[int, Field(description="The entry ID")]) -> EntryData:
...
def get_tag(id: Annotated[int, Field(description="The tag ID")]) -> TagData:
    """Get a tag by ID."""
    tag = db.get_tag(id)
    if tag is None:
        raise ValueError(f"Tag with ID '{id}' not found")
    return TagData(id=tag.id, name=tag.name, description=tag.description)
