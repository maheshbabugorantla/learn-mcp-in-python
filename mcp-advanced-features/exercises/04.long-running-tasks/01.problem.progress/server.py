"""EpicMe MCP server — long-running tasks: progress (problem).

Rendering a "year in review" wrapped video is slow work. A tool that just
blocks for seconds and then returns leaves the client blind — no way to show a
bar, no sign it's alive. MCP's progress notifications fix that: while the tool
runs, it calls `ctx.report_progress(...)` and the client receives each update.

The `create_wrapped_video` tool is the lesson. Everything else is the base
journal. The slow render itself lives in `video.py` (a fake — no ffmpeg); the
tool's one job is to feed its progress out to the client.
"""
...
# A real render would take minutes; we fake a short one so tests stay fast.
MOCK_SECONDS = 1.0


@mcp.tool()
async def create_wrapped_video(
    year: Annotated[int, Field(description="The year to summarize")], ctx: Context
) -> CallToolResult:
    """Render a "year in review" wrapped video, reporting progress as it goes."""

    # 🐨 the render can tell us how far along it is — wire that up to the client:
    #    define an on_progress callback that awaits
    #      ctx.report_progress(progress=fraction, total=1.0, message="Creating video...")
    #    and pass it to render_wrapped_video as on_progress=on_progress.
    # 💣 for now we render with NO progress callback, so the tool finishes and
    #    returns the uri but never reports a single step — the test's
    #    ">=2 progress events" assertion will fail until you wire it up.
    uri = await render_wrapped_video(year, MOCK_SECONDS)
