"""EpicMe MCP server — long-running tasks: cancellation (problem).
...
async def create_wrapped_video(
    ...
    # 🐨 make this tool react to cancellation: define an on_cancelled callback
    #    that sets `global LAST_CANCELLED; LAST_CANCELLED = True`, then pass it
    #    to render_wrapped_video as on_cancelled=on_cancelled.
    # 💣 right now we pass no on_cancelled, so when the caller cancels mid-render
    #    the cleanup path never runs and LAST_CANCELLED stays False — the test's
    #    "cleanup ran" assertion will fail until you wire it up.
