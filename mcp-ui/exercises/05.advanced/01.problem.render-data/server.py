"""EpicMe MCP server — render-data (problem).
...
def view_entry(
    ...
    if entry is None:
        raise ValueError(f"Entry with ID '{id}' not found")
    # 🐨 build a GENERIC iframe URL — no id in it:
    #    iframe_url = f"{BASE_URL}/ui/entry-viewer"
    # 🐨 return a UI resource that carries the entry as initial render data, so
    #    the page has it before first paint (no round trip to fetch it):
    #    create_ui_resource(
