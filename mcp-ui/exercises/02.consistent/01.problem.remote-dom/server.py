"""EpicMe MCP server — remote-dom (problem).
...
# 🐨 write a _tag_remote_dom_script(tag) that returns a JS string which:
#    - creates a 'ui-stack' element (document.createElement('ui-stack'))
#    - creates a 'ui-text' element with the tag name via setAttribute('content', ...)
#    - appends them, then root.appendChild(stack)
#    - for a missing tag, uses the text 'Tag not found'
# 💰 json.dumps(tag.name) gives you a safely-quoted JS string literal.
# 💣 once view_tag emits remote-dom below, nothing calls _tag_html anymore —
#    delete it (the solution does).
    ...
    return CallToolResult(
        content=[
            create_ui_resource(
                ...
                # 🐨 switch this from rawHtml to a remote-dom resource:
                #    - "type":      "remoteDom"
                #    - "framework": "react"
