...
# introspection response of `{"active": false}` doesn't match it and blows up.
#
# Make it a discriminated union of the two real shapes:
#
...
#       active: Literal[False]      # and nothing else — a dead token says no more
#
...
#
# Then handle the inactive branch down in `resolve_auth_info`.
...
async def resolve_auth_info(auth_header: str | None) -> AuthInfo | None:
    ...
    # TODO: An inactive token is not a usable token.
    #
    # `data` is now either an ActiveToken or an InactiveToken. If it isn't
    # active, return None — expired, revoked, and never-existed all end here.
