...
# TODO: Distinguish "no token" from "bad token".
#
# Both are 401s, but they aren't the same problem, and the client's next move
# differs. Add these two auth params **only when the request actually carried an
# Authorization header**:
#
#   error="invalid_token"
#   error_description="The access token is invalid or expired"
#
# Check with `had_a_token = "authorization" in request.headers`.
#
# Why the condition? Telling a client that its nonexistent token is invalid
# sends it off to refresh something it doesn't have. "You sent nothing" and
# "what you sent is stale" deserve different words.
