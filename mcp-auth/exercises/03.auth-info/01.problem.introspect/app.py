...
from auth import handle_oauth_authorization_server_request, handle_oauth_protected_resource_request, handle_unauthorized
...
# the header check with a real one, using the `resolve_auth_info` you just wrote:
#
...
async def authorize(request: Request) -> Response | None:
    ...
    This is a placeholder for the real thing: it only asks whether the client
    sent *an* Authorization header, not whether the token in it is any good.
    Anyone who types the word "Bearer" gets in. `03.auth-info` fixes that.
    """
