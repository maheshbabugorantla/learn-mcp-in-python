...
# ask who's calling — and it can't take an argument for it either, because the
# *model* fills in tool arguments and the model must not get to pick who it is.
#
...
#   )
#
# Then a `require_auth_info() -> AuthInfo` that returns it, and raises a
# RuntimeError if it's None. It shouldn't ever be None — `app.py` rejects
# tokenless requests before they get here — but if someone later mounts the MCP
# app without the auth middleware, you want a loud failure and not a journal
# served to nobody in particular.
#
# (This is the Cloudflare workshop's `ctx.props`, minus the Cloudflare.)
