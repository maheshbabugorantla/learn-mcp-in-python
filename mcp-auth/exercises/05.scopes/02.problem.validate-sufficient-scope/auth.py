...
#: Every scope this resource server understands. Clients read this list out of
#: the protected resource metadata and ask for what they need.
SUPPORTED_SCOPES = [
    "user:read",
    "entries:read",
    "entries:write",
...
#: The scope combinations that get you *something*. Ours are all single scopes,
#: because any one of them makes some part of the journal usable. A server whose
#: cheapest useful operation needed two scopes at once would list a pair here.
#:
#: This one's given to you — it's fiddly to describe and easy to read.
...
def has_sufficient_scope(auth_info: AuthInfo) -> bool:
    """True if this token can do *anything at all* here.

    Also given. Note it's `any`, not `all`: this asks whether the request is
    worth handling, not whether some particular tool is allowed.
    """
    return any(
...
# TODO: Turn away a token that can't do anything here.
#
# Write `def handle_insufficient_scope() -> Response` returning a **403** with a
# `WWW-Authenticate` header carrying these auth params:
#
#   Bearer realm="EpicMe"
#   error="insufficient_scope"
#   error_description="Any of the following combinations of scopes is valid: ..."
#
# Build the description from MINIMAL_VALID_SCOPE_COMBINATIONS — join each
# combination with spaces, and the combinations with commas — so the client is
# told what would actually work instead of guessing.
#
# Why 403 and not 401? A 401 means "go get a token" and a client will dutifully
# refresh and retry forever. A 403 means "that token will never work here" —
# the difference between a client that recovers and a client in a loop.
#
# (There's a `scope` auth param for naming required scopes, but it means "all of
# these", and here any one will do. The spec says don't demand more than you
# need, so the explanation goes in error_description instead.)
