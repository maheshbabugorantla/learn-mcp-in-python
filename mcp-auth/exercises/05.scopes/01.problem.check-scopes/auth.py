...
#
# Create a `SUPPORTED_SCOPES` list with these five, in this order:
#   "user:read", "entries:read", "entries:write", "tags:read", "tags:write"
#
# Then a `Scope` type so a typo is a type error rather than a permission that
...
#       ...True only if *every* scope in `scopes` is in `auth_info.scopes`.
#
# Every, not any — this answers "may they do this specific thing?", and a thing
# that needs two scopes needs both.
