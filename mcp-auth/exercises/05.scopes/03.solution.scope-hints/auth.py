...
def handle_oauth_protected_resource_request(request: Request) -> Response:
    """Describe this resource server: what it is, who vouches for it, what to ask for."""
    return JSONResponse(
        {
            "resource": str(request.url.replace(path="/mcp", query="")),
            "authorization_servers": [AUTH_SERVER_URL],
            # Without this, a client has to guess what to put in `scope=` when
            # it sends the user off to authorize. Guessing means asking for
            # everything, or asking for the wrong thing and failing later.
