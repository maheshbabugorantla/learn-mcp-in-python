...
#
# Add `"scopes_supported": SUPPORTED_SCOPES` to the JSON below.
#
# Without it, a client sending someone off to authorize has to guess what to put
# in `scope=`. Guessing means asking for everything (so the consent screen
# demands far more than the app needs) or asking for the wrong thing and failing
# later.
...
def handle_insufficient_scope() -> Response:
    """403. Who you are is fine. What you asked for isn't.

    401 and 403 are different questions, and clients treat them differently: a
    401 means "go get a token", a 403 means "that token will never work — get a
    different one". Sending a 401 here would put a client in a refresh loop
    against an authorization server that is behaving perfectly.
    """
    combinations = ", ".join(
        " ".join(combination) for combination in MINIMAL_VALID_SCOPE_COMBINATIONS
    )
    return Response(
        "Forbidden",
        status_code=403,
        headers={
            "WWW-Authenticate": ", ".join(
                [
                    ...
                    "error_description=\"Any of the following combinations of scopes"
                    f' is valid: {combinations}"',
                    # There's a `scope` auth param for naming what's required,
                    # but it means "you need all of these", and here any one
                    # will do. The spec says don't ask for more than you need,
                    # so the explanation goes in error_description instead.
                ]
            )
        },
    )
